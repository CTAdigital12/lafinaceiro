...
interface SpreadsheetReconciliationModalProps {
  open: boolean;
...
export function SpreadsheetReconciliationModal({
  ...
  const { createTransaction, deleteTransaction, updateTransaction } = useTransactions(month, year);
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(true);
    try {
      const [spreadsheetItems, systemTx] = await Promise.all([
        parseSpreadsheetFile(file),
        fetchSystemTransactions(),
      ]);

      if (spreadsheetItems.length === 0) {
        toast({ title: "Nenhum item encontrado na planilha", variant: "destructive" });
        setIsLoading(false);
        return;
      }
  ...
  const handleAddTransaction = useCallback(async (item: SpreadsheetItem) => {
    ...
    try {
      ...
      const systemTx = await fetchSystemTransactions();
      const newResult = reconcileSpreadsheet(
        result?.onlyInSpreadsheet.filter((s) => s.rowIndex !== item.rowIndex).concat(
  ...
  const handleDeleteTransaction = useCallback(async (tx: SystemTransaction) => {
    ...
    try {
      ...
      // Refresh
      const systemTx = await fetchSystemTransactions();
      // Rebuild spreadsheet items from current result
      const allSpreadsheetItems = [
        ...(result?.matched.map((m) => m.spreadsheet) || []),
        ...(result?.valueDiscrepancies.map((d) => d.spreadsheet) || []),
  ...
  }, [updateTransaction, fetchSystemTransactions, result, toast]);
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Conciliar com Planilha — {creditCardName}
            </DialogTitle>
          ...
          {!result ? (
            <div className="flex flex-col items-center justify-center py-16 gap-6">
              ...
              <div className="text-center space-y-2">
                <h3 className="font-semibold text-lg">Upload da Planilha do Banco</h3>
                <p className="text-sm text-muted-foreground max-w-md">
                  Faça upload do arquivo CSV ou XLSX com os lançamentos da fatura.
                  As colunas esperadas são: <strong>Data</strong>, <strong>Descrição</strong> e <strong>Valor</strong>.
                </p>
              </div>
              ...
              <div className="flex justify-between items-center pt-2 border-t">
                <label className="cursor-pointer">
                  ...
                  <Button variant="outline" size="sm" asChild>
                    <span><Upload className="h-3 w-3 mr-1" /> Nova Planilha</span>
                  </Button>
                </label>
